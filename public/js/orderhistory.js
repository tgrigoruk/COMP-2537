      ...
      if (orders.length) {
        ...
        for (i = 0; i < orders.length; i++) {
          ...
          let order = `
                        ...
                        <div>Order at ${time} for ${formatter.format(
            total
          )}</div>
                        <div class="order-items">
                        `;
          items.forEach((item) => {
            order += `<div class="order-item">• ${capitalize(item.name)} (${
              item.quantity
            } @ ${formatter.format(item.price)})</div>`;
          });
          order += "</div></div>";
          $("#orders").prepend(order);
        }
      } else {
        $("#orders").append(`<p class="empty-message">No past orders</p>`);
      }
    },
  });
}
